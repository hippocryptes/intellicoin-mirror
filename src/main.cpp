...
const string strMessageMagic = "intellicoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // intellicoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // intellicoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if(pindexBest->nHeight < 299959)
    {
    nSubsidy = 400 * COIN;
    }
    else if(pindexBest->nHeight < 299960)
    {
    nSubsidy = 200 * COIN;
    }
...
// intellicoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // intellicoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // intellicoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // intellicoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // intellicoin: track money supply and mint amount info
...
// intellicoin: total coin age spent in transaction, in the unit of coin-days.
...
// intellicoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // intellicoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // intellicoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // intellicoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // intellicoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // intellicoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // intellicoin: check proof-of-stake
    ...
    // intellicoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // intellicoin: getblocks may not obtain the ancestor block rejected
    ...
    // intellicoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "intellicoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "In Intellicoin we trust";
        CTransaction txNew;
        txNew.nTime = 1408177800;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1408177800;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2138461;
        if(fTestNet)
        {
            block.nNonce   = 1500873;
        }


        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc05dcef7cb218ea47484016c67d50be5eec2d536ac01bf086e084f0c942f6dc3"));
        ...
        // intellicoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x7f, 0x31, 0xe2, 0x05 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // intellicoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // intellicoin: tell downloading node about the latest block if it's
